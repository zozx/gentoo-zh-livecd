...
# Fetch Minimal ISO path
LATEST_ISO_PATH=$(curl -s "${MIRROR}/latest-iso.txt" | grep -v '^#' | grep "install-amd64-minimal" | awk '{print $1}' | head -n 1)

if [ -z "${LATEST_ISO_PATH}" ]; then
    echo "[!] Error: Could not determine latest ISO path from latest-iso.txt"
    exit 1
fi

# Fetch Stage3 path (try openrc first, fallback to standard stage3)
LATEST_STAGE3_PATH=$(curl -s "${MIRROR}/latest-stage3-amd64-openrc.txt" | grep -v '^#' | awk '{print $1}' | head -n 1)
if [ -z "${LATEST_STAGE3_PATH}" ]; then
    LATEST_STAGE3_PATH=$(curl -s "${MIRROR}/latest-stage3-amd64.txt" | grep -v '^#' | awk '{print $1}' | head -n 1)
fi

if [ -z "${LATEST_STAGE3_PATH}" ]; then
    echo "[!] Error: Could not determine latest Stage3 path"
    exit 1
fi

echo "  -> ISO URL: ${MIRROR}/${LATEST_ISO_PATH}"
echo "  -> Stage3 URL: ${MIRROR}/${LATEST_STAGE3_PATH}"

wget "${MIRROR}/${LATEST_ISO_PATH}" -O "${WORK_DIR}/gentoo-base.iso"
wget "${MIRROR}/${LATEST_STAGE3_PATH}" -O "${WORK_DIR}/stage3.tar.xz"
...
# Extract emerge, env-update, emaint, and related core tools/configs from Stage3 using --wildcards
tar -xf "${WORK_DIR}/stage3.tar.xz" -C "${SQUASH_DIR}" --wildcards \
    "usr/bin/emerge*" \
    "usr/sbin/env-update" \
    "usr/sbin/emaint" \
    "usr/sbin/etc-update" \
...
wget https://distfiles.gentoo.org/snapshots/portage-latest.tar.xz -O "${WORK_DIR}/portage-latest.tar.xz"
...
if [ -z "${VMLINUZ_URL}" ] || [ -z "${MODULES_URL}" ]; then
    echo "[!] Error: Failed to retrieve download URLs from gentoo-cjk-kernel release"
    exit 1
fi

wget "${VMLINUZ_URL}" -O "${WORK_DIR}/vmlinuz-cjk"
wget "${MODULES_URL}" -O "${WORK_DIR}/modules-cjk.tar.xz"
